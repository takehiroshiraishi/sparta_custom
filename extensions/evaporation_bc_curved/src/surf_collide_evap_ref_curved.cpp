...
#include "stdlib.h"
#include "string.h"
#include "surf_collide_evap_ref_curved.h"
#include "surf.h"
...
SurfCollideEvapRefCurved::SurfCollideEvapRefCurved(SPARTA *sparta,
                                                   int narg, char **arg) :
  SurfCollide(sparta, narg, arg)
{
  if (narg != 4)
    error->all(FLERR,"Illegal surf_collide evapref/curved command");
...
SurfCollideEvapRefCurved::~SurfCollideEvapRefCurved()
{
  if (copy) return;
...
void SurfCollideEvapRefCurved::init()
{
  SurfCollide::init();
  check_tsurf();
}
...
Particle::OnePart *SurfCollideEvapRefCurved::
