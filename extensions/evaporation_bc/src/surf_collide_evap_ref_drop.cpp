...
#include "stdlib.h"
#include "string.h"
#include "surf_collide_evap_ref_drop.h"
#include "surf.h"
...
SurfCollideEvapRefDrop::SurfCollideEvapRefDrop(SPARTA *sparta, int narg, char **arg) :
  SurfCollide(sparta, narg, arg)
{
  if (narg != 4) error->all(FLERR,"Illegal surf_collide evaprefdrop command");

  parse_tsurf(arg[2]);

  acc = input->numeric(FLERR,arg[3]);
  if (acc < 0.0 || acc > 1.0)
    error->all(FLERR,"Illegal surf_collide evaprefdrop command");
...
SurfCollideEvapRefDrop::~SurfCollideEvapRefDrop()
{
  if (copy) return;
...
void SurfCollideEvapRefDrop::init()
{
  SurfCollide::init();
  check_tsurf();
}
...
Particle::OnePart *SurfCollideEvapRefDrop::
        ...
        int isurf, double *norm, int isr, int &reaction)
{
  ...
  if (random->uniform() < acc) {
    ...
    if (persurf_temperature) {
      tsurf_local = t_persurf[isurf];
      if (tsurf_local <= 0.0)
        error->one(FLERR,"Surf_collide evaprefdrop tsurf <= 0.0");
    }
    ...
    double vrm = sqrt(2.0*update->boltz * tsurf_local / species[ispecies].mass);
    ...
    ip->erot = particle->erot(ispecies,tsurf_local,random);
    ip->evib = particle->evib(ispecies,tsurf_local,random);
  }
  return NULL;
}
